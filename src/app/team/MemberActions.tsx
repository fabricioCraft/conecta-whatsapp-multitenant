...
export default function MemberActions({ member, currentUserId }: { member: Member; currentUserId: string }) {
    const [loading, setLoading] = useState(false)

    // Não permitir ações no próprio usuário (opcional, mas boa prática de UX)
    if (member.id === currentUserId) return <span className="text-xs text-slate-500 italic">Você</span>
        ...
        <div className="flex items-center gap-2">
            {member.role === 'member' ? (
                ...
                <button
                    ...
                    disabled={loading}
                    className="p-2 rounded-lg bg-yellow-500/10 text-yellow-500 hover:bg-yellow-500/20 transition-colors disabled:opacity-50"
